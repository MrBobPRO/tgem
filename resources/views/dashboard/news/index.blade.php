...
<section class="search">
    <div class="select2_single_container">
        <select class="select2_single select2_single_linked" data-placeholder="Поиск..."
            ...
            <option value="{{ route('dashboard.news.single', $item->id)}}">{{$item->ruTitle}}</option>
            @endforeach
        </select>
    </div>
</section>
...
<section class="list">
    ...
    <div class="titles">
        <div class="titles__item width-25">
            ...
            <a class="titles__link"
                href="{{ route('dashboard.news.index') . '?page=' . $active_page . '&order_by=ruTitle&order_type=asc' }}">Заголовок
                <span class="material-icons-outlined titles__icon">arrow_upward</span>
            </a>
            @elseif($order_by == "ruTitle" && $order_type == "asc")
            <a class="titles__link"
                href="{{ route('dashboard.news.index') . '?page=' . $active_page . '&order_by=ruTitle&order_type=desc' }}">Заголовок
    ...
    <form action="{{ route('news.remove_multiple') }}" method="POST" id="multiple_items_form">
        ...
        <div class="list__item">
            ...
            <div class="list__item-div width-25">{{$new->ruTitle}}</div>
