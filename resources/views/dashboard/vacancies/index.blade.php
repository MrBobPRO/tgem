...
<div class="index-page-alert alert alert-primary">
    У изображение вакансии объязательно ширина и высота должны быть одинаковыми, чтобы сохранять пропорции !
</div>
...
<section class="search">
    <div class="select2_single_container">
        <select class="select2_single select2_single_linked" data-placeholder="Поиск..."
            ...
            <option value="{{ route('dashboard.vacancies.single', $item->id)}}">{{$item->ruTitle}}</option>
            @endforeach
        </select>
    </div>
</section>
...
<section class="list">
    {{-- Titles start --}}
    <div class="titles">
        <div class="titles__item width-50">
            @if($order_by != "ruTitle")
            <a class="titles__link"
                href="{{ route('dashboard.vacancies.index') . '?page=' . $active_page . '&order_by=ruTitle&order_type=asc' }}">Заголовок
                <span class="material-icons-outlined titles__icon">arrow_upward</span>
            </a>
            @elseif($order_by == "ruTitle" && $order_type == "asc")
            <a class="titles__link"
                href="{{ route('dashboard.vacancies.index') . '?page=' . $active_page . '&order_by=ruTitle&order_type=desc' }}">Заголовок
    ...
    <form action="{{ route('vacancies.remove_multiple') }}" method="POST" id="multiple_items_form">
        ...
        <div class="list__item">
            ...
            <div class="list__item-div width-50">{{$vacancy->ruTitle}}</div>
            @php
                $formatted = Carbon\Carbon::create($vacancy->created_at)->locale("ru");
            @endphp
            <div class="list__item-div width-50">{{ $formatted->isoFormat("DD MMMM YYYY") }}</div>
