...
<section class="search">
    <div class="select2_single_container">
        <select class="select2_single select2_single_linked" data-placeholder="Поиск..."
            ...
            <option value="{{ route('dashboard.vacancies.single', $item->id)}}">{{$item->title}}</option>
            @endforeach
        </select>
    </div>
</section>
...
<section class="list">
    ...
    <div class="titles">
        <div class="titles__item width-50">
            ...
            <a class="titles__link"
                href="{{ route('dashboard.vacancies.index') . '?page=' . $active_page . '&order_by=title&order_type=asc' }}">Заголовок
                <span class="material-icons-outlined titles__icon">arrow_upward</span>
            </a>
            @elseif($order_by == "title" && $order_type == "asc")
            <a class="titles__link"
                href="{{ route('dashboard.vacancies.index') . '?page=' . $active_page . '&order_by=title&order_type=desc' }}">Заголовок
    ...
    <form action="{{ route('vacancies.remove_multiple') }}" method="POST" id="multiple_items_form">
        ...
        <div class="list__item">
            ...
            <div class="list__item-div width-50">{{$vacancy->title}}</div>
            @php
